...
class ReenqueueSettingsParser {
    ...
    private void fillSettings(ReenqueueSettings.Builder builder, String name, String value) {
        try {
            switch (name) {
                case SETTING_REENQUEUE_RETRY_TYPE -> builder.withRetryType(parseReenqueueRetryType(value));
                case SETTING_REENQUEUE_RETRY_PLAN -> builder.withSequentialPlan(parseReenqueueRetryPlan(value));
                case SETTING_REENQUEUE_RETRY_DELAY -> builder.withFixedDelay(Duration.parse(value));
                case SETTING_REENQUEUE_RETRY_INITIAL_DELAY -> builder.withInitialDelay(Duration.parse(value));
                case SETTING_REENQUEUE_RETRY_STEP -> builder.withArithmeticStep(Duration.parse(value));
                case SETTING_REENQUEUE_RETRY_RATIO -> builder.withGeometricRatio(Long.valueOf(value));
                default -> {
                }
            }
        } catch (RuntimeException exc) {
    ...
    private static ReenqueueRetryType parseReenqueueRetryType(String type) {
        return switch (type) {
            case VALUE_REENQUEUE_RETRY_TYPE_MANUAL -> ReenqueueRetryType.MANUAL;
            case VALUE_REENQUEUE_RETRY_TYPE_FIXED -> ReenqueueRetryType.FIXED;
            case VALUE_REENQUEUE_RETRY_TYPE_SEQUENTIAL -> ReenqueueRetryType.SEQUENTIAL;
            case VALUE_REENQUEUE_RETRY_TYPE_ARITHMETIC -> ReenqueueRetryType.ARITHMETIC;
            case VALUE_REENQUEUE_RETRY_TYPE_GEOMETRIC -> ReenqueueRetryType.GEOMETRIC;
